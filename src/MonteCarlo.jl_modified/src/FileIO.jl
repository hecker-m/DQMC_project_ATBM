...
"""
function save(filename::String, mc::MonteCarloFlavor; 
...
@bm function load(filename::String, groups::String...)
    ...
    output = try
        ...
        #=
        if file["VERSION"] == 1
            _git = ( branch = "master",  commit = "a4dbd321f551e6adc079370b033555ba9a2f75e5",  dirty = false)
        else
            _git = file["git"]
        end
        =#
        output = try 
            if haskey(file, "MC") && isempty(groups)
                ...
                println(isempty(groups), "..and went to else")
