mutable struct StandardFieldCache{T1, T2, T3, T4, T5} <: AbstractFieldCache
    Δ::T1
    R::T2
    invRΔ::T2
    IG::T3
    G::T3
    IG_G::T5    #I added this to make vsubkron! faster for ComplexF64. 
                #If everyting would work for StructArray one could use @turbo instead, 
                #and remove this cache field.
    detratio::T4
    eVop1::T1
    eVop2::T1
    eVop3::T1
...
function FieldCache(field, model)
    ...
    if flv == 1
        ...
        if greens_matrix_type(field, model) <: BlockDiagonal
            ...
            IG_G  = MT(undef, N*flv, N*flv)
        end
        
    end
    eVop1=MT(undef, flv, flv)
    eVop2=MT(undef, flv, flv)
    eVop3=MT(undef, flv, flv)

    detratio = zero(GET)

    return StandardFieldCache(Δ, R, invRΔ, IG, G, IG_G, detratio,eVop1,eVop2,eVop3)
end
...
function update_greens!(cache::StandardFieldCache, G, i, N)
    # calculate M_Δ=cache.invRΔ =  Δ R⁻¹
    vldiv22!(cache, cache.R, cache.Δ)
    
    # calculate Left=cache.IG = (I - G)[:, i:N:end]
    vsub!(cache.IG, I, G, i, N)

    # calculate Right=cache.G = M_Δ * G[i:N:end, :]
    vmul!(cache.G, cache.invRΔ, G, i, N)

    # update greens function 
    # G[m, n] -= {(I - G)[m, i:N:end]} {M_Δ * G[i:N:end, n]}
    vsubkron!(G, cache.IG, cache.G, cache.IG_G)
...
function ConfigRecorder(::Type{<: Union{AbstractHirschField, AbstractGHQField}}, rate = 10)
    ConfigRecorder{BitArray}(rate)
end

function Base.push!(c::ConfigRecorder, field::Union{AbstractHirschField, AbstractGHQField}, sweep)
    (sweep % c.rate == 0) && push!(c.configs, compress(field))
    nothing
end

function BufferedConfigRecorder(::Type{<: Union{AbstractHirschField, AbstractGHQField}}, filename; rate = 10, chunk_size = 1000)
    BufferedConfigRecorder{BitArray}(filename, rate, chunk_size)
end

function Base.push!(cr::BufferedConfigRecorder, field::Union{AbstractHirschField, AbstractGHQField}, sweep)
