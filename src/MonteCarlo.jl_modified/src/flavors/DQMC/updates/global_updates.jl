...
@bm function propose_global_from_conf(mc::DQMC, m::Model, field::AbstractField)
    ...
    #current_slice=1 ensures that we computed G[0,0] ≡ G_ϕ before, 
    #and we gonna compute  G'[0,0] ≡ G'_ϕ below
    ...
    #saves the previous singular values into mc.stack.tempvf, probably sorted ?
    #after calculating greens, Dl holds σν^(-1), as said above
    ...
    inv_det(mc, current_slice(mc)-1, field)     #writes the (inverted) singular values into mc.stack.Dr, probably sorted ?
...
# That statement is misleading. temp_conf actually holds the original configurartion. conf holds the proposed one.
