...
"""
function local_sweep(mc::DQMC, model::Model)
    accepted = 0
...
"""
@bm function sweep_spatial(mc::DQMC, m::Model)
    N = length(m.l)

    accepted = 0
    @inbounds for i in 1:N

        detratio, ΔE_boson, new_op = propose_local(mc, m, field(mc), i, current_slice(mc))
        p = exp(- ΔE_boson) * detratio
        #if i==N println("det= ", detratio, " ,ΔE_boson= ", ΔE_boson, " ,p=", p) end
        ...
        if real(p) > 1 || rand() < real(p)
            accept_local!(mc, m, field(mc), i, current_slice(mc), detratio, ΔE_boson, new_op)
            accepted += 1
        end
    end
...
@bm update(::LocalSweep, mc::DQMC, model, field) = local_sweep(mc, model) /length(mc.model.l)/ 2nslices(mc)
