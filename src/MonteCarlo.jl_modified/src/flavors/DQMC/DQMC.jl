...
function DQMC(model::M;
    ...
    rand!(field_data, range=random_kick)
...
function initialize_run(mc; 
    ...
    th_groups = generate_groups(
        mc, mc.model, 
        [mc.thermalization_measurements[k] for k in keys(mc.thermalization_measurements) if !(k in ignore)]
    )
    ...
    indx_local=[];
    indx_global=[];
    for (k, update) in enumerate(mc.scheduler.sequence)
        if typeof(update.update)==LocalSweep
            push!(indx_local, k)
        elseif typeof(update.update)==GlobalConstMove
            push!(indx_global, k)
        end
    end
...
@bm function run!(
    ...
    th_groups, groups, indx_local, indx_global = initialize_run(mc; 
        verbose = verbose, ignore = ignore,
        thermalization = thermalization, sweeps = sweeps
    )
    ten_register=zeros(ten_reg);idx_ten=0;  # Used for averaging over last ~ten global accepts, for more precise Δ_global_box 

    while mc.last_sweep < total_sweeps
        verbose && (mc.last_sweep == thermalization + 1) && println("\n\nMeasurement stage - ", sweeps)

        if mc.last_sweep==thermalization && measure_decay_τ  
            rand!(mc.field, range=random_kick)
            reverse_build_stack(mc, mc.stack)
            propagate(mc)
        end
        ...
        if mc.last_sweep >= next_print
            ...
            if verbose || mc.last_sweep ∈ Iterators.flatten([10, 20, thermalization+10, thermalization+20, thermalization+500:500:total_sweeps])
                println("\t", mc.last_sweep)
        ...
        #Update Δ_box length for local and global sweeps (only during thermalization)
        if typeof(mc.field)<:AbstractContMBF
            if mc.last_sweep ≤ thermalization && mod(mc.last_sweep,10)==0
                loc_acc_rate::Float64 = 0.0;   
                for idx in indx_local    loc_acc_rate +=mc.scheduler.sequence[idx].accepted/max(1,mc.scheduler.sequence[idx].total) end      
                loc_acc_rate=loc_acc_rate/length(indx_local)   

                verbose && println("Now, at sweep ", mc.last_sweep)
                if loc_acc_rate < 0.5
                    verbose && @printf("\t\tshrinking box: %.2f\n", 0.9*mc.parameters.box_local)
                    mc.parameters.box_local = 0.9 * mc.parameters.box_local
                else
                    verbose && @printf("\t\tenlarging box: %.2f\n", 1.1*mc.parameters.box_local)
                    mc.parameters.box_local = 1.1 * mc.parameters.box_local
                end
                for idx in indx_local
                mc.scheduler.sequence[idx].accepted=0.0
                mc.scheduler.sequence[idx].total=0
                end 
            end  

            # global updates adjustment
            if mc.last_sweep ≤ thermalization && !isempty(indx_global) && 
                mod(mc.last_sweep,10*div(length(mc.scheduler.sequence), length(indx_global)))==0
                #is called after every 10 global updates
                    global_acc_rate_temp ::Float64 =0.0;
                    for idx in indx_global
                        global_acc_rate_temp +=mc.scheduler.sequence[idx].accepted/max(1,mc.scheduler.sequence[idx].total)      
                    end
                    #we average over the last ten global acceptance rates
                    idx_ten +=1;
                    ten_register[1+mod(idx_ten-1,length(ten_register))] = global_acc_rate_temp/length(indx_global);
                    global_acc_rate=sum(ten_register)/min(idx_ten, length(ten_register));
                    if global_acc_rate < 0.5
                        verbose && @printf("\t\tshrinking box_global: %.2f\n", 0.99*mc.parameters.box_global)
                        mc.parameters.box_global = 0.99 * mc.parameters.box_global
                    else
                        verbose && @printf("\t\tenlarging box_global: %.2f\n", 1.01*mc.parameters.box_global)
                        mc.parameters.box_global = 1.01 * mc.parameters.box_global
                    end
                    for idx in indx_global
                        mc.scheduler.sequence[idx].accepted=0.0
                        mc.scheduler.sequence[idx].total=0
    ...
    # Total timings
    if verbose || mc.last_sweep==total_sweeps-1
