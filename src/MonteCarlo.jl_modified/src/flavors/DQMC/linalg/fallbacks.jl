...
function vmul!(C::AbstractArray, A::AbstractArray, B::AbstractArray)
    ...
    #matmul!(C, A, B) 
    mul!(C, A, B)       
    ###I switched mul!() to matmul!() from the Octavian package.
    #God knows why, but this works significantly faster on the cluster!!
end
...
#O is a temp. Computing A=A * Permut(pivot) * inverse(T::UpperTriangular)
