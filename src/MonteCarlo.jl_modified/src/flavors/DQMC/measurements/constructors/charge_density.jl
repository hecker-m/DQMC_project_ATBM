...
"""
@inline Base.@propagate_inbounds function full_cdc_kernel(mc::DQMC, model, ij::NTuple{2}, 
    G::Union{GreensMatrix, _GM4{T}}, flv) where {T <: Matrix}
    return full_cdc_kernel(mc, model, ij, G, flv, field(mc))
end


@inline Base.@propagate_inbounds function full_cdc_kernel(mc, model, ij, G::GreensMatrix, flv, field::AbstractField)
    return full_cdc_kernel(mc, model, ij, (G, G, G, G), flv, field)
end
...
N = length(lattice(mc))
id = I[i, j] * I[G0l.k, G0l.l] * I[α, αp]

return (1 - Gll.val[i+(α-1)*N, i+(α-1)*N]) * (1 - G00.val[j+(αp-1)*N, j+(αp-1)*N]) +
        Gl0.val[i+(α-1)*N, j+(αp-1)*N]*(id - G0l.val[j+(αp-1)*N, i+(α-1)*N])
        #I don`t know why in G0l.val[j+s1, i+s2]) Gl0.val[i+s1, j+s2], s1 and s2 were suddenly swapped.
...
@inline Base.@propagate_inbounds function full_cdc_kernel(
        ...
        packed_greens::_GM4{<: DiagonallyRepeatingMatrix}, flvs, ::AbstractField
    )
    i, j = ij
	G00, G0l, Gl0, Gll = packed_greens
...
@inline Base.@propagate_inbounds function full_cdc_kernel(
        ::DQMC, ::Model, ij::NTuple{2}, packed_greens::_GM4{<: BlockDiagonal}, flv, ::AbstractField
    )
    i, j = ij
    f1, f2 = flv
	G00, G0l, Gl0, Gll = packed_greens    
...
@inline Base.@propagate_inbounds function reduced_cdc_kernel(
	...
	G00, G0l, Gl0, Gll = packed_greens
    N = length(lattice(mc))
return  -G0l.val[j+(αp-1)*N, i+(α-1)*N] * Gl0.val[i+(α-1)*N, j+(αp-1)*N]

end
@inline Base.@propagate_inbounds function reduced_cdc_kernel_old(mc, model, ij, G::GreensMatrix, flv)
    return reduced_cdc_kernel_old(mc, model, ij, (G, G, G, G), flv)
end

@inline Base.@propagate_inbounds function reduced_cdc_kernel_old(
        mc, ::Model, ij::NTuple{2}, packed_greens::_GM4{<: Matrix}, flv
    )
    i, j = ij
    f1, f2 = flv
