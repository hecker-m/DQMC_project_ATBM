...
function add_default_measurements!(mc; kwargs...)
    ...
    ##I added the following to get FullBinner
    obs = FullBinner()

     # autocomplete measurements
     get!(kwarg_dict, :occ, occupation(mc, mc.model))
     get!(kwarg_dict, :Mx, magnetization(mc, mc.model, :x, obs=obs))
     get!(kwarg_dict, :My, magnetization(mc, mc.model, :y, obs=obs))
     get!(kwarg_dict, :Mz, magnetization(mc, mc.model, :z, obs=obs))
     get!(kwarg_dict, :G, greens_measurement(mc, mc.model, obs=obs))
     get!(kwarg_dict, :K, kinetic_energy(mc, mc.model, obs=obs))
     get!(kwarg_dict, :V, interaction_energy(mc, mc.model, obs=obs))
     get!(kwarg_dict, :E, total_energy(mc, mc.model, obs=obs))
 
     get!(kwarg_dict, :CDC, charge_density_correlation(mc, mc.model, obs=obs))
     get!(kwarg_dict, :PC, pairing_correlation(mc, mc.model, obs=obs))
     get!(kwarg_dict, :SDCx, spin_density_correlation(mc, mc.model, :x, obs=obs))
     get!(kwarg_dict, :SDCy, spin_density_correlation(mc, mc.model, :y, obs=obs))
     get!(kwarg_dict, :SDCz, spin_density_correlation(mc, mc.model, :z, obs=obs))
 
     get!(kwarg_dict, :CDS, charge_density_susceptibility(mc, mc.model, obs=obs))
     get!(kwarg_dict, :PS, pairing_susceptibility(mc, mc.model, obs=obs))
     get!(kwarg_dict, :SDSx, spin_density_susceptibility(mc, mc.model, :x, obs=obs))
     get!(kwarg_dict, :SDSy, spin_density_susceptibility(mc, mc.model, :y, obs=obs))
     get!(kwarg_dict, :SDSz, spin_density_susceptibility(mc, mc.model, :z, obs=obs))
     get!(kwarg_dict, :CCS, current_current_susceptibility(mc, mc.model, obs=obs))
    ##I added until here
    ...
    for (k, v) in kwarg_dict
        #mc[k] = v
        mc.measurements[k] =v
        mc.thermalization_measurements[k] =v
        #measurements(mc, stage=:ME)[k]=v
        #measurements(mc, stage=:TH)[k]=v
    end
