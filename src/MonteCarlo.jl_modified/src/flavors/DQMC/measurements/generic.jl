...
struct DQMCMeasurement{F <: Function, GI, LI, FI, OT, T} <: AbstractMeasurement
    ...
    flavor_iterator::FI
    kernel::F
    observable::OT #default data-(strorage)-type is a LogBinner
    temp::T
end
...
"""
_default_capacity(mc::DQMC) = 8 * ceil(Int, mc.parameters.sweeps / mc.parameters.measure_rate)
...
@bm function apply!(::Greens, combined::Vector{<: Tuple}, mc::DQMC)
    G = maybe_repeating(mc, greens!(mc)) # generates G(0,0, _greens!())
    for (lattice_iterator, measurement) in combined
        prepare!(lattice_iterator, measurement, mc)
    end
...
@bm function apply!(iter::TimeIntegral, combined::Vector{<: Tuple}, mc::DQMC)
    ...
    for (i, (g0l, gl0, gll)) in enumerate(init(mc, iter))
        ...
        for (lattice_iterator, measurement) in combined
            ...
            #for lattice iterators (Time Integral), time integration happens on m.temp; m.temp is then a field over distances. 
            # see mc.model.l[:Bravais_srctrg2dir] where l=lattice
        end
    ...
    for (lattice_iterator, measurement) in combined
         finish!(lattice_iterator, measurement, mc)  
        #for lattice iterators, this is where the push!(m.observable, m.temp) happens
        #if lattice_iterator, then potentially divided by  N=length(lattice(mc))=L^2  
    end
            ...
            for b2 in 1:B, b1 in 1:B
                ...
                for trg in 1:N
                    @simd for src in 1:N
                        ...
                        temp[dir, b1, b2] += weight * measurement.kernel(mc, mc.model, (src + uc1, trg + uc2), packed_greens, σ)
                    end
                end
            end
        end
...
function apply!(
    ...
    @timeit_debug "apply!(::EachBondPairByBravaisDistance, ::$(typeof(measurement.kernel)))" begin
        ...
        @inbounds @fastmath for σ in measurement.flavor_iterator    
                            #iterates over two flavor indices, σ is a tuple.
...
@inline function commit!(::AbstractLatticeIterator, m) 
    push!(m.observable, m.temp)
end
#@inline commit!(::Sum, m) = push!(m.observable, m.temp[1])
@inline function commit!(::Sum, m)
