mutable struct DQMCStack{
    ...
    Mxs::Array{Number,3}    #σx [sx sy sz]
    pnys::Array{Number,3}   #[σ0 σz] sy
...
function initialize_stack(mc::DQMC, ::DQMCStack)
    ...
    #mc.stack.eV .= zero(interaction_eltype(field(mc))) ;#I added this to ensure eV is initialized with zeros, rather than undefined~10^(-100).
    mc.stack.eye_flv=Matrix{Float64}(I, flv, flv)
    mc.stack.Mxs=Mx_matrices(GreensElType)
    mc.stack.pnys=pny_matrices(nothing)
    nothing
end

function pny_matrices(::Nothing)
    pny_s=zeros(Float64,2,4,4);
    σ0=[1 0; 0 1];σz=[1 0; 0 -1];
    syt=[0 -1; 1 0];    #note that sy=im*syt
    pny_s[1,:,:]=kron(σ0, syt)
    pny_s[2,:,:]=kron(σz, syt)
    return pny_s
end

function Mx_matrices(GreensElType)
        Mx_Ms=zeros(ComplexF64,3,4,4);
        Mx_Ms[1,1,4]=1;
        Mx_Ms[1,2,3]=1;
        Mx_Ms[1,3,2]=1;
        Mx_Ms[1,4,1]=1;
        ...
        Mx_Ms[3,3,1]=1;
        Mx_Ms[3,4,2]=-1;
        if GreensElType==Float64
            return real(Mx_Ms) #this makes the matrices to Float64, but Mx_y=0. Mx_y should not occur in your theory.
        else
            return Mx_Ms
        end
end
