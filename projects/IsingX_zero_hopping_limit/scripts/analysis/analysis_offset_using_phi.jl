...
my_keys=[(key=:SDC_A1, q=(0,0)), (key=:SDS_A1, q=(0,0)), 
    (key=:NemC, ), (key=:NemS, ), 
    (key=:A1p_dQ_C, ), (key=:A1p_dQ_S, ), (key=:h4ϕ, ),(key=:h4ϕ_OnSite, ),
    (key=:E_potϕ,),(key=:Sspin_2_A1_xx, ), (key=:Sbil_2_B1_xx, ),(key=:Sbil_2_A1p_xx, )]

df_ϕ_cols=(L=Int[], T=Float64[], U=Float64[], B=Int[], δτ=Float64[]);
...
df_ϕ_OP_cols=(L=Int[], T=Float64[], U=Float64[], B=Int[], δτ=Float64[]);
...
if length(Binder_keys)>0
    df_Binder_cols=(L=Int[], T=Float64[], U=Float64[], B=Int[], δτ=Float64[]);
...
for _para in eachindex(paras)
    ...
    ############
    global my_values=get_observable_using_ϕ(dqmcs,  Val(:Q0πQπ0_offset))
    println("done with set $(_para)a")
    vec=[L, T, U, Int(peierls), δτ]
    for ν in [2, 3, 6, 7, 10, 11, 13, 14, 15, 4, 8, 12]
    ...
    if length(Binder_keys)>0
        vec_Binder=[L, T, U, Int(peierls), δτ]
        for n in eachindex(Binder_keys)
