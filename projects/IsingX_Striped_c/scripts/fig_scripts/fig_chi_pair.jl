...
for (idx, (Ts, Ls)) in enumerate(zip(df_LT[:,:T],df_LT[:,:L]))
    ...
    eb=errorbars!(top, dfs[!,:U], dfs[!,:PDS_s_00], dfs[!,:ΔPDS_s_00]; linewidth=er_lw, 
    ...
    CairoMakie.scatterlines!(top, rangeU, χ_pair_00_analytic.(rangeU, 1/Ts, μ0_plot, Ls) ;  
...
top=Axis(fig[1, 1], xlabel=L"$U/t$", ylabel=L"$χ_{\mathrm{pair}}^{00} [\mathbf{Q}=(π,0)]$", ylabelsize=30,
    xlabelsize=30, xticklabelsize=20, yticklabelsize=20, title=my_title
)

for (idx, (Ts, Ls)) in enumerate(zip(df_LT[:,:T],df_LT[:,:L]))
    dfs=filter(:L=>L -> L==Ls ,filter(:T =>T ->T==Ts, df))  

    eb=errorbars!(top, dfs[!,:U], dfs[!,:PDS_s_π0], dfs[!,:ΔPDS_s_π0]; linewidth=er_lw, 
        whiskerwidth=10,color = colorschemes[:coolwarm][1-1/Ts/βmax]
    )
    CairoMakie.translate!(eb, 0, 0, -0.5)
...
for (idx, (Ts, Ls)) in enumerate(zip(df_LT[:,:T],df_LT[:,:L]))
    ...
    eb=errorbars!(top, dfs[!,:U], dfs[!,:PDS_spm_00], dfs[!,:ΔPDS_spm_00]; linewidth=er_lw, 
        whiskerwidth=10,color = colorschemes[:coolwarm][1-1/Ts/βmax]
    )
    CairoMakie.translate!(eb, 0, 0, -0.5)
    ...
    CairoMakie.scatterlines!(top, rangeU, χ_pair_ZZ_analytic.(rangeU, 1/Ts, μ0_plot, Ls) ;  
        marker =:xcross, markersize=1, linestyle=:dot, linewidth=1.5,
        color = colorschemes[:coolwarm][1-1/Ts/βmax]
    )
end
...
for (idx, (Ts, Ls)) in enumerate(zip(df_LT[:,:T],df_LT[:,:L]))
    ...
    eb=errorbars!(top, dfs[!,:U], dfs[!,:PDS_spm_π0], dfs[!,:ΔPDS_spm_π0]; linewidth=er_lw, 
        whiskerwidth=10,color = colorschemes[:coolwarm][1-1/Ts/βmax]
    )
    CairoMakie.translate!(eb, 0, 0, -0.5)
...
for (idx, (Ts, Ls)) in enumerate(zip(df_LT[:,:T],df_LT[:,:L]))
    ...
    i1=1+(idx-1)*2
    i2=2+(idx-1)*2
    eb=errorbars!(top, dfs[!,:U], dfs[!,:PDS_XX_00], dfs[!,:ΔPDS_XX_00]; linewidth=er_lw, 
        whiskerwidth=10,color = colorschemes[:rainbow][i1/(2nTs)]
    )
    CairoMakie.translate!(eb, 0, 0, -0.5)
    CairoMakie.scatterlines!(top, dfs[!,:U], dfs[!,:PDS_XX_00] ;  marker = :xcross,
        color = colorschemes[:rainbow][i1/(2nTs)], label = L"\mathbf{Q}=(0,0), β=%$(1/Ts)", markersize=10
    )
    eb=errorbars!(top, dfs[!,:U], dfs[!,:PDS_XX_π0], dfs[!,:ΔPDS_XX_π0]; linewidth=er_lw, 
        whiskerwidth=10,color = colorschemes[:rainbow][i2/(2nTs)]
    )
    CairoMakie.translate!(eb, 0, 0, -0.5)
    CairoMakie.scatterlines!(top, dfs[!,:U], dfs[!,:PDS_XX_π0] ;  marker = :xcross,
        color = colorschemes[:rainbow][i2/(2nTs)], label = L"\mathbf{Q}=(π,0), β=%$(1/Ts)", markersize=10
    )
...
for (idx, (Ts, Ls)) in enumerate(zip(df_LT[:,:T],df_LT[:,:L]))
    ...
    nTs=length(df_LT[!, :T]);
    i3=1+(idx-1)*1
    eb=errorbars!(top, dfs[!,:U], dfs[!,:PDS_YYzz_00], dfs[!,:ΔPDS_YYzz_00]; linewidth=0.5, 
        whiskerwidth=10,color = colorschemes[:rainbow][i3/(1nTs)]
    )
    CairoMakie.translate!(eb, 0, 0, -0.5)
    CairoMakie.scatterlines!(top, dfs[!,:U], dfs[!,:PDS_YYzz_00] ;  marker = :xcross,
        color = colorschemes[:rainbow][i3/(1nTs)], label = "β=$(1/Ts)", markersize=10
    )     
end
axislegend( position=(1, 1))
...
for (idx, (Ts, Ls)) in enumerate(zip(df_LT[:,:T],df_LT[:,:L]))
    ...
    CairoMakie.scatterlines!(top, dfs[!,:U], dfs[!,:PDS_YYzz_π0] ;  marker = :xcross,
        color = colorschemes[:rainbow][i4/(1nTs)], label = "β=$(1/Ts)", markersize=10
    )        
end
axislegend( position=(1, 1))
...
for (idx, (Ts, Ls)) in enumerate(zip(df_OP_LT[:,:T],df_OP_LT[:,:L]))
    ...
    CairoMakie.scatterlines!(top, df_OPs[!,:U], df_OPs[!,Symbol(mkey)] ;  marker = :xcross,
        color = colorschemes[:tab20][idx], label = "T=$(round(Ts, digits=2)), L=$Ls", markersize=10
    )       
end
axislegend( position=(1,1))
...
for (idx, (Us, Ls)) in enumerate(zip(df_OP_LU[:,:U],df_OP_LU[:,:L]))
    ...
    CairoMakie.scatterlines!(top, df_OPs[!,:T], df_OPs[!,Symbol(mkey)] ;  marker = :xcross,
        color = colorschemes[:tab20][idx], label = "U=$(round(Us, digits=2)), L=$Ls", markersize=10
    )       
end
axislegend( position=(1,1))
