...
for (idx, (Ts, Ls)) in enumerate(zip(df_LT[:,:T],df_LT[:,:L]))
    ...
    CairoMakie.lines!(top, range(Umin, Umax,nU), [χ_B1_nem_XY(1/Ts, U/Nϕ, muM, Ls) for U in range(Umin, Umax,nU)] ;  
            linestyle = :dash,   color = colorschemes[:coolwarm][1-1/Ts/βmax]) 
    CairoMakie.lines!(top, range(Umin, Umax,nU), [χ_B1_nem_Z(1/Ts, U/Nϕ, muM, Ls) for U in range(Umin, Umax,nU)] ;  
            linestyle = :dash,   color = colorschemes[:coolwarm][1-1/Ts/βmax]) 
end
...
for (idx, (Ts, Ls)) in enumerate(zip(df_LT[:,:T],df_LT[:,:L]))
    ...
    CairoMakie.lines!(top, range(Umin, Umax,nU), [S_B1_nem_XY(1/Ts, U/Nϕ, muM, Ls) for U in range(Umin, Umax,nU)] ;  
            linestyle = :dash,   color = colorschemes[:coolwarm][1-1/Ts/βmax]) 
    CairoMakie.lines!(top, range(Umin, Umax,nU), [S_B1_nem_Z(1/Ts, U/Nϕ, muM, Ls) for U in range(Umin, Umax,nU)] ;  
            linestyle = :dash,   color = colorschemes[:coolwarm][1-1/Ts/βmax])
end
#axislegend( position=(0, 1))
